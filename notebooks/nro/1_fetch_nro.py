...
tasks = [] # prepare tasks -> ultimately should be a list of dicts in the other url: file_name_to_save_resp_as
for row in rows:
    ...
    if len(cells) == 7: # there are other tables on the page but ours is 7 cells wide
        ...
        if link is not None:
            ...
            link = link.strip()
            tasks.append({'url': f'{base_url}{link}', 'file_name': output_folder / f'{date}_{name}_{id}.pdf'})
...
print(f'Executed {len(tasks)} URL fetch tasks. Run time: {datetime.utcnow() - start_time}')